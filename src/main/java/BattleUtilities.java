...
public class BattleUtilities {
   ...
   public  static Map<String, Vendor> decideInitiative(Vendor vendorOne, Vendor vendorTwo){
    ...
    public static void checkVitals(Vendor vendorOne, Vendor vendorTwo) throws BattleIsOverException {
        if ((BattleUtilities.vendorIsDead(vendorOne) || BattleUtilities.vendorIsDead(vendorTwo))) {

            throw new VendorHasDiedException();
        }
        if((!BattleUtilities.vendorCanAttack(vendorOne) || !BattleUtilities.vendorCanAttack(vendorTwo))){
            throw new VendorHasDepletedAttacksException();
        }
    }
